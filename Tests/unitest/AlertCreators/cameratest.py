...
class CameraTest(unittest.TestCase):
    ...
    def test_createAlert_humanOutside2_systemCreateAlert(
    ...
    def test_detectMovement_connected_createAlert(self, create_alert_mock):
        self.connectedCamera.detect_movement(
            "human", "outside", datetime(2020, 11, 26, 13, 20)
        )
    ...
    def test_detectMovement_nightTimeNmodeon_createAlert(self, create_alert_mock):
        ...
        self.connectedCamera.detect_movement(
            "human", "outside", datetime(2020, 11, 26, 23, 20)
        )
    ...
    def test_detectMovement_unknownLocation_raise(self):
        with self.assertRaises(Exception):
            self.connectedCamera.detect_movement(
                "human", "inside", datetime(2020, 11, 26, 13, 20)
            )
